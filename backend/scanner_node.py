...
class ScannerNode:
    ...
    def __init__(self) -> None:
        ...
        self.heartbeat_subscriber = self._node.make_subscriber(uavcan.node.Heartbeat_1_0, 7509)
        self.heartbeat_subscriber.receive_in_background(self.heartbeat_callback)

        self.port_subscriber = self._node.make_subscriber(uavcan.node.port.List_1_0, 7510)
        self.port_subscriber.receive_in_background(self.port_callback)

        self.all_nodes:                 List[NodeInfo] = [NodeInfo(node_id=node_id) for node_id in range(127)]  # Holds information about all Cyphal nodes
        self.subject_attributes:        Dict[int, List[str]] = {}                                               # Holds information about all subject attributes, for example [{1245}:(position, velocity, acceleration), ...]
        self.publishers_subscribers:    Dict[int, pycyphal.application.Subscriber] = {}
        self.message_timestamps:        Dict[int, List[float]] = {}                                             # Dictionary "wheel" to calculate each subject message rate (Hz) 

        self.message_queue      = asyncio.Queue(maxsize=200)                                                    # ToDo: test performance on weak hardware like Pi 3
        self.active_publishers  = {}                                                                            # Dictionary where the keys are subject_ids and the values are sets. 
                                                                                                                # Each set contains the node IDs that publish to that specific subject.
        self.services  = []
    ...
    async def update_reg_list(self, node_id: int) -> tuple[dict[int, str], dict[int, str]]:
        """
        Read node registers (only if they were registered) and return dictionaries where subject/service IDs are keys
        and message/service types are their values.
        For example: ({1235: 'reg.udral.physics.kinematics.translation.Linear_0_1', 1236: 'uavcan.primitive.String_1_0'},
                    {123: 'dontpanic.SumService_1_0'})
        
        Args:
            node_id (int): The ID of the node to query.
        
        Returns:
            tuple[dict[int, str], dict[int, str]]: Two dictionaries, one for subject IDs and message types,
                                                one for service IDs and service types.
        """
        ...
        try:
            ...
            while True:
                ...
                list_response_tuple = await asyncio.wait_for(
                    self.register_list_client.call(list_request), timeout=2.0
                )
            ...
            for reg_name in register_names:
                ...
                if re.search(r'\.pub\..*\.id$', reg_name):
                    ...
                    type_access_response_tuple = await asyncio.wait_for(
                        self.register_access_client.call(type_access_request), timeout=5.0
                    )
                ...
                # Process service registers (uavcan.srv.<service_id>.type)
                    ...
                    # Validate service ID (0–511 for Cyphal services)
                    if not (0 <= service_id <= 511):
            ...
            # Step 3: Check for standard services (optional, if not in registers)
            for standard_service_id, service_type in [
                ...
                if standard_service_id not in dsdl_srv_messages:
                    ...
                    logging.info(f"Node {node_id} standard service {standard_service_id}: {dsdl_srv_messages[standard_service_id]}")

            logging.debug(f"Node {node_id} has registered messages: {dsdl_pub_messages}, services: {dsdl_srv_messages}")
            print(dsdl_srv_messages)
    ...
    async def add_subscriptions(self, node_id: int, dsdl_pub_messages: Dict) -> None:
        ...
        for subject_id, message_type in dsdl_pub_messages.items():
            ...
            attribute_list = [attribute.name for attribute in message_type_model.attributes]
            ...
            if subject_id not in self.publishers_subscribers:
                ...
                logging.info(f"Subscribed to publisher on subject {subject_id}")

    async def add_servers(self, node_id: int, dsdl_srv_messages: Dict) -> Dict[int, Dict]:
        """
        ...
        for service_id, service_type in dsdl_srv_messages.items():
            try:
                ...
                logging.info(f"Created client for service {service_type} (ID: {service_id}) on node {node_id}")
                ...
                if client_key not in self.service_clients:
                    ...
                    logging.info(f"Created and cached client for service {service_type} (ID: {service_id}) on node {node_id}")
    ...
    async def make_service_call(self, node_id: int, service_id: int, service_type: str, attributes: Dict[str, Dict[str, Any]]) -> str:
        ...
        try:
            ...
            try:
                response_tuple = await asyncio.wait_for(client.call(request), timeout=5)
                logging.debug(f"Received response tuple for service {service_id}: {response_tuple}")
            except asyncio.TimeoutError:
                logging.error(f"Service call timed out for service {service_id} on node {node_id} after 5 seconds")
                raise Exception(f"Service {service_id} on node {node_id} timed out after 5 seconds")
    ...
    def _find_non_none_field(self, union_value: uavcan.register.Value_1_0) -> tuple[str | None, Any | None]:
        """
        Helper method to extract the non-none field from a uavcan.register.Value_1_0 union.

        Args:
            union_value (uavcan.register.Value_1_0): The register value union.

        Returns:
            tuple[str | None, Any | None]: The field name and its value, or ('empty', None) if empty.
        """
        field_names = [
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if not list_response_tuple or not list_response_tuple[0]:
                        logging.error(f"List request timed out for node_id={node_id}, index={index}")
                        list_response_tuple = await asyncio.wait_for(
                            self.register_list_client.call(list_request), timeout=2.0
                            )
                        if not list_response_tuple or not list_response_tuple[0]:
                    ...
                    try:
                        ...
                        if not access_response_tuple or not access_response_tuple[0]:
                            ...
                            if not access_response_tuple or not access_response_tuple[0]:
                                logging.error(f"Access request failed for register '{register_name}' on node_id={node_id}")
                                index += 1
                                continue
    ...
    async def _publisher_callback(self, msg, transfer: pycyphal.transport.TransferFrom, subject_id: int) -> None:
        # populates message_queue with the data
        timestamp           = time.time()
        timestamp_non_unix  = datetime.datetime.fromtimestamp(timestamp).strftime('%Y-%m-%d %H:%M:%S')
        ...
        self.message_timestamps[subject_id] = [t for t in self.message_timestamps[subject_id] if t > timestamp - 10] # Window of 10 seconds is used. 
        rate = round(self.get_message_rate(subject_id))

        # Extracts values for each attribute detected in a message type
        attributes = []
        for attribute in self.subject_attributes[subject_id]:
            attr_value = getattr(msg, attribute, None)
            if attr_value is None:
                continue
            # Decodes byte arrays into strings
            if isinstance(attr_value, (bytes, bytearray, np.ndarray)):
                try:
                    decoded_value = bytes(attr_value).decode("utf-8", errors="ignore")
                    logging.debug(f"{attribute}: {decoded_value}")
                    attributes.append({"attribute": attribute, "value": decoded_value})
                except Exception as e:
                    logging.error(f"Failed to decode {attribute}: {e}")
            else:
                numeric_value   = None
                unit_name       = None
                for sub_attr in dir(attr_value):
                    if not sub_attr.startswith("_"):
                        sub_value = getattr(attr_value, sub_attr, None)
                        if isinstance(sub_value, (int, float)):
                            numeric_value   = sub_value
                            unit_name       = sub_attr
                            break

                if numeric_value is not None:
                    logging.debug(f"{attribute}: {numeric_value}")
                    logging.debug(f"Unit: {unit_name}")
                    attributes.append({"attribute": attribute, "value": numeric_value, "unit": unit_name})

                elif isinstance(attr_value, (str, int, float, bool)):
                    logging.debug(f"{attribute}: {attr_value}")
                    attributes.append({"attribute": attribute, "value": attr_value})

                else:
                    logging.debug(f"Skipping non-primitive attr_value for {attribute}: {attr_value}")
        
        try:
            self.message_queue.put_nowait((subject_id, timestamp_non_unix, rate, msg.__class__, attributes))
        except asyncio.QueueFull:
            logging.warning("Message queue full, dropping oldest message")
            self.message_queue.get_nowait()  # Remove oldest
            await self.message_queue.put((subject_id, timestamp_non_unix, rate, msg.__class__, attributes))

    async def set_register(self, node_id: int, register_name: str, value: str, type: str) -> Optional[Any]:
        """
    ...
    async def port_callback(self, msg: uavcan.node.port.List_1_0, transfer: pycyphal.transport.TransferFrom):
        ...
        self.all_nodes[node_id].setPortList(port_list=msg)
    ...
    async def heartbeat_callback(self, msg: uavcan.node.Heartbeat_1_0, transfer: pycyphal.transport.TransferFrom):
        ...
        if not node.has_appeared:
            node.hasAppeared(first_seen=datetime.datetime.now())
            logging.debug(f"Node {node_id} has appeared for the first time.")
            await self.getInfo(node_id)
            
        else:
            # Check if the appeared node is the same as the disappeared one. If not - refresh node info
            old_unique_id = ''.join(f'{byte:02x}' for byte in node.unique_id) if node.has_responded_to_getInfo else "Unknown"
            await self.getInfo(node_id)
            new_unique_id = ''.join(f'{byte:02x}' for byte in node.unique_id) if node.has_responded_to_getInfo else "Unknown"

            if node.has_responded_to_getInfo and old_unique_id != new_unique_id:
                logging.debug(f"Node {node_id} has a new unique_id: {new_unique_id}. Resetting.")
                self.all_nodes[node_id] = NodeInfo(node_id=node.node_id) 
                await self.getInfo(node_id)

            node.wasSeen(last_seen=datetime.datetime.now())
    ...
    async def getInfo(self, node_id: int) -> None:
        ...
        try:
            ...
            self.all_nodes[node_id].setInfo(getInfo_response=response[0], transfer_from=response[1])
