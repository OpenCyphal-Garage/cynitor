...
where the `public_regulated_data_types` is a git-submodule referencing [Cyphal's official DSDL files](https://github.com/OpenCyphal/public_regulated_data_types) and `custom_messages` which contains our own message definitions.
...
the `.h`-files you can include in your C/C++ project contained in `dsdl_messages/include` and


```shell
sh python_generate.sh
```
...
the `.py`-files you can include in your python project contained in `dsdl_messages/py_message_modules`. Clean up the generated files with `cleanup.sh`.